...
Original file is located at
    https://colab.research.google.com/drive/1mO-8x_uxnaqYSZD15xo8R-hval0bpKmh
"""
...
# token text with images and their respective five captions
...
# image nams along with caption in dictionary
def imageid_nd_captionsplit(token_text):
  ...
  for each_row in token_text.split('\n'):
    ...
    if im not in imageid_nd_caption.keys():
      imageid_nd_caption[im]=[]
    imageid_nd_caption[im].append(image_caption)
...
# data cleaning to remove special characters, alpha numeric, single characters
import string
import copy
...
#sample caption after cleaning
imageid_nd_caption_cleaned['1000268201_693b08cb0e']

#storing all captions to a file to avoind re doing
...
# extracting total distinct words from the captions
...
#total path for images
images=[all_images_path + file_name for file_name in images]

images

#extracting traindata_path from above
...
print("Total Train Data: "+str(len(train_data_fullpath)))

#extracting test data full path
test_data_fullpath=trainDataFullPath(images,test_data)

test_data_fullpath[:10]

#total test data
print("Total Test Data: "+str(len(test_data_fullpath)))

#appending start and end tags to captions and storing in dictionary
...
print("length of training captions: "+str(len(train_datacaption)))
...
#image pre-processing to reshape and put in ann array
def image_preprocessing(imagedirectory):
    x=299
    y=299
    after_resize = image.load_img(imagedirectory, target_size=(x, y))
    arrayedimg = image.img_to_array(after_resize)
    arrayedimg = np.expand_dims(arrayedimg, axis=0)
    arrayedimg = preprocess_input(arrayedimg)
    return arrayedimg
...
#storing test and train data feture to a google drive file
import pickle
...
#list of all training captions for the model
...
print("all captions for training data: "+str(len(list_ofalltrainingcaptions)))
...
# reducing vocabulary to so to increase computational efficiency
...
#dictionaries having index of words and words of index
...
# calculationg max caption length so to fix caption input size
max_caption=0
for each_caption in list_ofalltrainingcaptions:
  if max_caption<len(each_caption.split()):
    max_caption=len(each_caption.split())

# padding so that each input will be of size 34 when sending word by word
...
#obtaing vector representation of words
import numpy as np
embedding_file=open("/content/gdrive/My Drive/Machine Learning/Flickr_Data/Flickr_Data/glove.6B.200d.txt",encoding="utf-8")
word_embedded_indices={}
for each_row in embedding_file:
...
# more epoch were used but kept changing the same code part
epoch_count = 50
batch_size = 900
...
test_encodedfeatures['2077079696_03380d218b.jpg'].shape

test_encodedfeatures

test_encodedfeatures['2077079696_03380d218b.jpg'].shape

print(word_from_index)

#greedy search algorithm to predict captions
...
len(test_encodedfeatures)

[*test_encodedfeatures][999]
...
for image in test_encodedfeatures.keys():
  ...
  plt.show()
  print(image)
  print(greedy_search(input_image))
  img=mpimg.imread('/content/gdrive/My Drive/Machine Learning/Flickr_Data/Flickr_Data/Images/'+image)
  imgplot = plt.imshow(img)
...
#beam search algorithm to predict captions with varible beam size
...
for image in test_encodedfeatures.keys():
  ...
  plt.show()
  print(image)
  print(prediction_using_beamsearch(input_image,3))
  print("--------------------------------------")

#bleu score calculations below
...
def blue_score_evaluation_with_sentence_bleu(imageid_nd_caption_cleaned,test_encodedfeatures,bcount):
  predictedscore=0
  len=0
  for image in test_encodedfeatures.keys():
    machinecaption=[]
    allcaptionforblue=[]
    input_image=test_encodedfeatures[image].reshape((1,2048))
    machinecaption=prediction_using_beamsearch(input_image,bcount)
...
alltrainimages_features

print("BLEU score of the train predicted caption_for_900_2_49: "+str(blue_score_evaluation_with_sentence_bleu_train(imageid_nd_caption_cleaned, alltrainimages_features,3)))

print("BLEU score of the train caption_greedy_for_900_2_49: "+str(blue_score_evaluation_with_sentence_bleu_greedy(imageid_nd_caption_cleaned, alltrainimages_features)))

print("BLEU score of the predicted caption_for_900_2_49: "+str(blue_score_evaluation_with_sentence_bleu(imageid_nd_caption_cleaned, test_encodedfeatures,3)))

print("BLEU score of the predicted caption_greedy_for_900_2_49: "+str(blue_score_evaluation_with_sentence_bleu_greedy(imageid_nd_caption_cleaned, test_encodedfeatures)))

print("BLEU score of the predicted caption_for_900_2_49_for_beamcount_5: "+str(blue_score_evaluation_with_sentence_bleu(imageid_nd_caption_cleaned, test_encodedfeatures,5)))

training_m.load_weights('/content/gdrive/My Drive/Machine Learning/Flickr_Data/Flickr_Data/weights_512/weights49.h5')

print("BLEU score of the predicted caption_for_512_49_for_beamcount_3: "+str(blue_score_evaluation_with_sentence_bleu(imageid_nd_caption_cleaned, test_encodedfeatures,3)))

print("BLEU score of the predicted caption_for_512_49_for_beamcount_5: "+str(blue_score_evaluation_with_sentence_bleu(imageid_nd_caption_cleaned, test_encodedfeatures,5)))

print("BLEU score of the predicted caption_greedy_for_500_49: "+str(blue_score_evaluation_with_sentence_bleu_greedy(imageid_nd_caption_cleaned, test_encodedfeatures)))

training_m.load_weights('/content/gdrive/My Drive/Machine Learning/Flickr_Data/Flickr_Data/weights49.h5')
...
training_m.load_weights('/content/gdrive/My Drive/Machine Learning/Flickr_Data/Flickr_Data/weights0.h5')
...
for image in test_encodedfeatures.keys():
  ...
  for caption in imageid_nd_caption_cleaned[inputimage]:
    ...
    actualCaptions.append(tempList)
  # print(actualCaptions)
  pred=prediction_using_beamsearch(input_image,3)
  predictionscore = sentence_bleu(actualCaptions,pred.split(' '))
  print("Image_Name: "+image)
  print("Predicted caption:"+pred)
  print("blue score when used batch size 900 and with >200 epochs: "+str(predictionscore))
  print("--------------------------------------")
...
for image in test_encodedfeatures.keys():
  ...
  for caption in imageid_nd_caption_cleaned[inputimage]:
    ...
    actualCaptions.append(tempList)
  # print(actualCaptions)
  pred=greedy_search(input_image)
  predictionscore = sentence_bleu(actualCaptions,pred.split(' '))
  print("Image_Name: "+image)
  print("Predicted caption with greedy search:"+pred)
  print("blue score when used batch size 900 and with >200 epochs: "+str(predictionscore))
  print("--------------------------------------")
